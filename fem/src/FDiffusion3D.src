...
! *
! *       Date of modification: 19 June 2001
! *
! ****************************************************************************/
...
!    --------------
     ...
     CALL SetDirichletBoundaries( Model, StiffMatrix, ForceVector, &
          TRIM(Solver % Variable % Name) // ' 3', 3, &
             Solver % Variable % DOFs, SomeQuantityPerm )

     CALL SetDirichletBoundaries( Model, StiffMatrix, ForceVector, &
          TRIM(Solver % Variable % Name) // ' 4', 4, &
             Solver % Variable % DOFs, SomeQuantityPerm )

     CALL SetDirichletBoundaries( Model, StiffMatrix, ForceVector, &
          TRIM(Solver % Variable % Name) // ' 5', 5, &
             Solver % Variable % DOFs, SomeQuantityPerm )

     CALL SetDirichletBoundaries( Model, StiffMatrix, ForceVector, &
          TRIM(Solver % Variable % Name) // ' 6', 6, &
             Solver % Variable % DOFs, SomeQuantityPerm )
...
!#if 1
    ...
    BRer => SomeQuantity(1:6*LocalNodes-5:6)
    BImr => SomeQuantity(2:6*LocalNodes-4:6)
    BRez => SomeQuantity(3:6*LocalNodes-3:6)
    BImz => SomeQuantity(4:6*LocalNodes-2:6)
    BRep => SomeQuantity(5:6*LocalNodes-1:6)
    BImp => SomeQuantity(6:6*LocalNodes:6)
...
!------------------------------------------------------------------------------
       DO p=1,NBasis
          DO q=1,NBasis
             ...
             DO i=1,dim
                A(i,i) = DCMPLX( M, D ) * Basis(q) * Basis(p)
             END DO
          ...
          DO i=1,dim
             LFORCE(dim*(p-1)+i) = LFORCE(dim*(p-1)+i) + s * Basis(p) * DCMPLX( LRe(i),LIm(i) )
          END DO
...
! and the external AC field may have all components in any coordinate system
     ...
     REAL(KIND=dp) :: ddBasisddx(MAX_NODES,3,3),B(3),dHdx(3,3),L(3)
...
!------------------------------------------
     ...
     ALLOCATE( BacRe(3,MAX_NODES), BacIm(3,MAX_NODES) )
