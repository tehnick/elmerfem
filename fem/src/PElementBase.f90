...
!------------------------------------------------------------------------------
      ...
      INTEGER, OPTIONAL :: localNumbers(3)
      ! Variables
      REAL (KIND=dp) :: La, Lb, Lc, Lb_La, Lc_1, Lb_Laj, Lc_1n
      REAL (KIND=dp), DIMENSION(2) :: dLa, dLb, dLc, grad
      INTEGER :: local(3)
    ...
    FUNCTION dTriangleEBubblePBasis(i,j,u,v,localNumbers) RESULT(grad)
      ...
      INTEGER, OPTIONAL :: localNumbers(3)
      ! Variables
      REAL (KIND=dp) :: La, Lb, Lc, Lb_La, Lc_1, Legi, Legj
      REAL (KIND=dp), DIMENSION(2) :: dLa, dLb, dLc, grad
      INTEGER :: local(3)
...
!------------------------------------------------------------------------------
      ...
      INTEGER, DIMENSION(4), OPTIONAL :: localNumbers
      ! Variables
      REAL(KIND=dp) :: La, Lb, Lc, Lha, Lhc, Lb_La, Lc_1, Legi, Legj, parW 
...
!------------------------------------------------------------------------------
    ...
    ! FUNCTION isPTriangle checks if given triangle is a p 
    ! element triangle.
...
!------------------------------------------------------------------------------
      ...
      IF ((elementCode == 303 .OR. elementCode == 306) .AND. &
           ASSOCIATED(Element % PDefs)) THEN
         retVal = .TRUE.
      END IF
...
!------------------------------------------------------------------------------
      ...
      retVal = .FALSE.
      ! Check elementcode and p element flag
      IF (Element % Type % ElementCode == 504 .AND. &
          ASSOCIATED(Element % PDefs)) THEN
         retVal = .TRUE.
      END IF
...
!------------------------------------------------------------------------------
      ...
      retVal = .FALSE.
      ! Check elementcode and p element flag
      IF (Element % Type % ElementCode == 706 .AND.&
          ASSOCIATED(Element % PDefs)) THEN
         retVal = .TRUE.
      END IF
...
!------------------------------------------------------------------------------
      ...
      retVal = .FALSE.
      ! Check elementcode and p element flag
      IF (Element % Type % ElementCode == 605 .AND.&
          ASSOCIATED(Element % PDefs)) THEN
         retVal = .TRUE.
      END IF
    ...
    FUNCTION getNumberOfGaussPoints( Element, Mesh ) RESULT(ngp)
      ...
      INTEGER :: edgeP, faceP, bubbleP, ngp
      ...
      ! Element bubble p
      bubbleP = 0
      IF (Element % BDOFs > 0) bubbleP = Element % PDefs % P

      ! Get number of Gauss points. Number needed is 2*max(p)=2*(max(p)+1)/2
      ngp = MAX(1, edgeP, faceP, bubbleP)
      ngp = (ngp+1)
      ngp = ngp ** Element % Type % Dimension
    ...
    FUNCTION getNumberOfGaussPointsFace( Face, Mesh ) RESULT(ngp)
      ...
      ngp = (MAX(edgeP, Face % PDefs % P) + 1) ** 2
    END FUNCTION
