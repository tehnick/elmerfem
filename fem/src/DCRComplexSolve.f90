...
! *
! *       Date of modification: 14.11.2001
! *
! ****************************************************************************/

!------------------------------------------------------------------------------
SUBROUTINE DCRComplexSolver( Model,Solver,dt,TransientSimulation )
  !DEC$ATTRIBUTES DLLEXPORT :: DCRComplexSolver
!------------------------------------------------------------------------------
!******************************************************************************
!
...
!      --------------------------------
       DO p=1,NBasis
          DO q=1,NBasis
             A = DCMPLX( A0r, A0i ) * Basis(q) * Basis(p)
             DO i=1,dim
                A = A + DCMPLX( A1r(i), A1i(i) ) * dBasisdx(q,i) * basis(p)
                DO j=1,dim
                   DO k = 1,dim
...
!------------------------------------------------------------------------------
       DO p=1,n
          DO q=1,n
             A = DCMPLX( B0r, B0i ) * C0 * Basis(p)*Basis(q)
             A = A + SUM( C1(1:dim) * dBasisdx(q,1:dim) ) * Basis(p)
             LSTIFF(p,q) = LSTIFF(p,q) + s * A
          END DO
          LFORCE(p) = LFORCE(p) + s * Basis(p) * C0 * DCMPLX( L1, L2 ) 
       END DO
!------------------------------------------------------------------------------
    END DO
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     REAL(KIND=dp) :: u, v, w, s, detJ, EdgeBasis(MAX_NODES), Basis(MAX_NODES), &
         ...
         Pressure(2,MAX_NODES), FluxReal(MAX_NODES), FluxImag(MAX_NODES), &
         ResidualReal, ResidualImag, WaveFlux(2), Work(MAX_NODES)

     REAL(kind=dp) :: Bvector(3,MAX_NODES), BScalarReal(MAX_NODES), BscalarImag(MAX_NODES)
     REAL(kind=dp) :: B1(3), B0r, B0i, Greal, Gimag

     REAL(KIND=dp) :: Source, ResidualNorm, Area, Flux(MAX_NODES)
...
!     REAL(KIND=dp), POINTER :: Hwrk(:,:,:)
     ...
     REAL(KIND=dp) :: Grad(3,3), Normal(3), EdgeLength, Jump, JumpReal, JumpImag, &
                x(MAX_NODES),y(MAX_NODES),z(MAX_NODES),GradReal(3,3),GradImag(3,3)
     ...
     REAL(KIND=dp) :: u, v, w, s, detJ, EdgeBasis(MAX_NODES), &
          Basis(MAX_NODES), dBasisdx(MAX_NODES,3), &
             ...
             Pressure(2,MAX_NODES)

     REAL(KIND=dp) :: Residual, ResidualNorm, Area, Flux(MAX_NODES)

     REAL(kind=dp) :: Amatrix(3,3,MAX_NODES), AvectorReal(3,MAX_NODES), &
          AvectorImag(3,MAX_NODES), AscalarReal(MAX_NODES), AscalarImag(MAX_NODES)
...
!------------------------------------------------------------------------------
     ...
     REAL(KIND=dp) :: NodalSource(2,MAX_NODES), &
          Pressione(2,MAX_NODES), Work(MAX_NODES), &
          ResidualReal, ResidualImag
