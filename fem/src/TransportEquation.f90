...
!------------------------------------------------------------------------------
   ...
   !DEC$ATTRIBUTES DLLEXPORT :: TransportEquationSolver
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
      DO i = 1, AdvectionVariableComponents
        WRITE(VariableName,'(A,A,I1)') TRIM(Solver % Variable % Name),' ', i
        CALL SetDirichletBoundaries( Model, StiffMatrix, ForceVector, &
            VariableName, i, AdvectionVariableComponents, UPerm )
      END DO
...
!------------------------------------------------------------------------------
      CALL SolveSystem( StiffMatrix, ParMatrix, ForceVector, &
          U, Norm, AdvectionVariableComponents, Solver )
...
!------------------------------------------------------------------------------
     SUBROUTINE AverageAccelerationMethod( N, dt, MassMatrix, DampMatrix, &
         StiffMatrix, Force, X, V, A )
!      DLLEXPORT AverageAccelerationMethod
!------------------------------------------------------------------------------
       INTEGER :: N
