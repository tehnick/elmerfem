...
! *                       Date: In 2001
! *
! *              Modifications: 1.8.2002 / Peter Råback
! *
! *****************************************************************************/
    
!------------------------------------------------------------------------------
SUBROUTINE FreeSurfaceReduced( Model,Solver,dt,TransientSimulation )
!DEC$ATTRIBUTES DLLEXPORT :: FreeSurfaceReduced
!------------------------------------------------------------------------------
!******************************************************************************
!
!  Solve the equation resulting from drawing of viscous fibers or sheets
!
!  ARGUMENTS:
!
!  TYPE(Model_t) :: Model,  
...
!------------------------------------------------------------------------------
     ...
     DO Surface = 1,NoFreeSurfaces
       ...
       DO t=tfirst,Model%NumberOfNodes+tfirst-1
         ...
         ! Integrate from the fixed boundary until the desired flus is exceeded
         i = istart
         DO 
           v1x = v2x
           v1y = v2y
           ...
           IF(axisymmetric) THEN
             df = a*(x2**2.0-x1**2.0)/2.0d0 + b*(x2**3.0-x1**3.0)/3.0d0
           ELSE
             df = a*(x2-x1) + b*(x2**2.0-x1**2.0)/2.0d0
           END IF
