...
/*--------------------------------------------------------------------------*/
   ...
   if ( (Function = dlsym( Handle,NewName ) ) == NULL )
   {
...
#endif // HAVE_LOADLIBRARY_API
#endif // HAVE_DLOPEN_API 
...
#ifdef WIN32
     ( char *cmd,int l1,char *Value,int l2,int *len )
#else
#  if defined(UNICOS)
     ( char *cmd,int l1,char *Value,int l2,int *len )
#  else
#     if defined(AIX)
     ( char *cmd,char *Value,int *len )
#     else
     ( char *cmd,char *Value,int *len )
#     endif
#  endif
