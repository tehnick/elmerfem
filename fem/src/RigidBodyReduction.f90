...
!/*****************************************************************************
! *
! * Module for reducing a rigid body as a mass point for the linear elasticity
! *
! *****************************************************************************
! *
...
! *                       Date:  13 Sep 2002
! *
! *                  Module by:  Antti Pursula
! *
! *          Modification date:  27 Aug 2003 (AP)
! *
! ****************************************************************************/
!------------------------------------------------------------------------------
INTEGER FUNCTION RigidBody( Model, Solver, A, b, x, n, DOFs, Norm )
  !DEC$ATTRIBUTES DLLEXPORT :: RigidBody
!------------------------------------------------------------------------------
!******************************************************************************
!
!  ARGUMENTS:
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=dp), POINTER :: f(:), u(:), u2(:), Work(:)
  REAL(KIND=dp) :: CPUtime, TotTime, at, s
  ...
  CHARACTER(LEN=MAX_NAME_LEN) :: VersionID = "$Id: RigidBodyReduction.f90,v 1.13 2004/07/29 08:09:45 apursula Exp $"
...
!------------------------------------------------------------------------------
  IF ( .NOT. AllocationsDone ) THEN
    IF ( ListGetLogical( GetSimulation(), 'Output Version Numbers', GotIt ) ) THEN
      CALL Info( 'RigidBody', 'RigidBodyReduction version:', Level = 0 ) 
      CALL Info( 'RigidBody', VersionID, Level = 0 ) 
      CALL Info( 'RigidBody', ' ', Level = 0 ) 
    END IF
  END IF
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
     ...
     ALLOCATE( Work( Solver % Mesh % MaxElementNodes ) )
     ...
     DO t = Model % NumberOfBulkElements + 1, Model % NumberOfBulkElements &
        ...
        DO i = 1, Model % NumberOfBCs
           IF ( CurrentElement % BoundaryInfo % Constraint == &
              ...
              Work(1:m) = ListGetReal( Model % BCs(i) % Values, &
                   TRIM(VarName) // ' 1',m,NodeIndexes, gotIt )
              IF ( .NOT. GotIt )   Work(1:m) = &
                   ListGetReal( Model % BCs(i) % Values, &
                   TRIM(VarName) // ' 2',m,NodeIndexes, gotIt )
              IF ( .NOT. GotIt )   Work(1:m) = &
                   ListGetReal( Model % BCs(i) % Values, &
                   TRIM(VarName) // ' 3',m,NodeIndexes, gotIt )
              IF ( .NOT. GotIt )  CYCLE

              LBody = CurrentElement % BoundaryInfo % LBody
              RBody = CurrentElement % BoundaryInfo % RBody


              IF ( ANY( RigidIndex == LBody ) .OR. ANY( RigidIndex == RBody )) &
                   THEN
...
!------------------------------------------------------------------------------
                 ...
                 IF ( k > 0 ) THEN
                    ...
                    DO i = 1, DOFs
                       ...
                       IF ( A % FORMAT == MATRIX_SBAND ) THEN
                          CALL SBand_SetDirichlet( A, b, k+i, Work(1) )
                       ELSE IF ( A % FORMAT == MATRIX_CRS .AND. &
                            A % Symmetric ) THEN
                          CALL CRS_SetSymmDirichlet( A, b, k+i, Work(1))
                       ELSE
                          s = A % Values( A % Diag(k+i) )
                          b(k+i) = Work(1) * s
                          CALL ZeroRow( A, k+i )
                          CALL SetMatrixElement( A, k+i, k+i, 1.0d0 * s )
                       END IF
     ...
     IF ( ListGetLogical( Solver % Values, 'Output Node Types', GotIt ) ) THEN
        ...
        DummySolver => Solver
        NodeOutput => NodeTypes(:)
        CALL VariableAdd( Solver % Mesh % Variables, Solver % Mesh, &
             DummySolver, 'Node Type', 1, NodeOutput )
     END IF
...
!------------------------------------------------------------------------------
  ...
  A_Trans => CRS_Transpose( BMatrix, EigAnal, ASSOCIATED( BMatrix % DampValues ) )
...
!  CALL Info( 'RigidBody', Message, Level=5 )
  ...
  BB_Trans => CRS_Transpose( BB, EigAnal, ASSOCIATED( BMatrix % DampValues ) )
...
!------------------------------------------------------------------------------
  SUBROUTINE ComputeMassCenter( Centers, Model, Solver, Index, n )
!------------------------------------------------------------------------------
!******************************************************************************
!
!  DESCRIPTION:
...
!
!  INTEGER :: Index( Model % NumberOfBodies )
...
!------------------------------------------------------------------------------
    ...
    TYPE(Solver_t) :: Solver
    REAL(KIND=dp), POINTER :: Centers(:,:)
    INTEGER :: n, INDEX( Model % NumberOfBodies )

    TYPE(GaussIntegrationPoints_t), TARGET :: IntegStuff
    TYPE(Element_t), POINTER :: CurrentElement
    TYPE(Nodes_t) :: ElementNodes
    REAL(KIND=dp), POINTER :: Basis(:), dBasisdx(:,:), ddBasisddx(:,:,:)
...
!------------------------------------------------------------------------------
    ...
    DO t = 1, Solver % NumberOfActiveElements
       ...
       DO i = 1, Model % NumberOfBodies
          IF ( CurrentElement % BodyId == INDEX(i) ) THEN
             Rigid = .TRUE.
             EXIT
          END IF
       END DO
...
!------------------------------------------------------------------------------
    ...
    DEALLOCATE( ElementNodes % x, ElementNodes % y, ElementNodes % z, &
         Density, Basis, dBasisdx, ddBasisddx )
...
!------------------------------------------------------------------------------
  FUNCTION CRS_Transpose( A, MVal, DVal ) RESULT(B)
!------------------------------------------------------------------------------
!
