...
#else
  int egmain(char *prefix,int inmethod0,Real relh,mesh_t *mesh)
#endif
{
#if EXE_MODE
  ...
  int nogrids,nomeshes,nofile,dim,gotgrid;
  Real mergeeps;
...
#else
  printf("inmethod = %d %.3le\n",inmethod,relh);
  printf("prefix = %s\n",prefix);

  inmethod = inmethod0;
  strcpy(eg.filesin[0],prefix);
  outmethod = 0;

  if(inmethod == 0) {
    errorstat = LoadCommands(prefix,&eg,grids,1,IOmethods,info);
    inmethod = eg.inmethod;
    outmethod = eg.outmethod;
    info = !eg.silent;  
  }
 ...
 read_another_file:    
  ...
  case 1: 
	...
	CreateElmerGridMesh(&eg,&(grids[k]),&(data[k]),boundaries[k],relh,info);
	gotgrid = TRUE;
      }
    }
    LoadCommands(eg.filesin[nofile],&eg,grids,2,IOmethods,info); 
    break;
  ...
  for(k=0;k<nomeshes;k++) {
    ...
    if(gotgrid && grids[k].reduceordermatmax) {
      eg.reduce = TRUE;
  ...
  if(1) {
  printf("setting nodes\n"); 

  mesh->nodes = data[0].noknots;
  ...
  {  
    int elemdim =  GetMaxElementDimension(&data[0]);
  printf("setting elements in %ddim\n",elemdim); 


  if(elemdim == 3) {
    mesh->elements = data[0].noelements;
    ...
    mesh->boundaryelements = data[0].noelements;
