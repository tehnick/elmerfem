...
boundaries, setting up PDE-systems to solve, and exporting model data and results
for ElmerSolver and ElmerPost. There is also an internal postprocessor, which can be
used to plot color surfaces, contours, vector fields, and visualize time dependent data.
...
One of the main features of ElmerGUI is the interface to the parallel solver engine.
...
customize the interface for an proprietary application. An example of customizing the
menus is provided in appendix A.

ElmerGUI relies on the Qt4 cross platform framework \cite{QtHome} and it uses the Qwt5 libray
\cite{QwtHome} for technical applications to monitor the solution. The internal postprocessor is based on VTK (Visualization Toolkit) \cite{VTKHome}. The program is capable of using Tetgen \cite{TetgenHome} and Netgen \cite{NetgenHome} as external finite element mesh generators.
...
\end{verbatim}
\end{footnotesize}
\noindent This will retrieve the current development version of the Elmer-suite.

Bofore staring to compile, please make sure that you have the development packages of Qt4 and Qwt5 installed on your system (i.e., libraries, headers, and program development tools). Qt version 4.2 or higher is recommended. If you want to compile the internal postprocessor, then the development packages of VTK version 5 or higher are also required.
...
Finally, once the build process has finished, it suffices to set up the environment variable {\tt ELMERGUI\_HOME}, add it to {\tt PATH}, and copy the file ``ElmerGUI'' and the directory ``edf'' in this location:
\begin{verbatim}
...
modules, which may or may not be available depending on the installation.
