...
The final task is to choose an installation directory, {\tt ELMERGUI\_HOME}, add it to
environment variables, and copy the executable ``ElmerGUI.exe'' and the directory ``edf''
there. Finally, {\tt ELMERGUI\_HOME} should be added to {\tt PATH}.
...
\end{verbatim}
\end{footnotesize}
In this case, $A(2)=3$ and $A(4)=5$. Values between the points are interpolated linearly. The number
of points defining the interpolant may be arbitrary. 
