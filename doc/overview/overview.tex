...
This manual gives an overview of the Elmer software package. The emphasis is on big things
...
equations. Elmer includes models for a large number of different equations which may be coupled 
...
The original development consortium included partners from CSC - The Finnish IT Center 
for Science, Helsinki University of Technology TKK, VTT Technical Research Centre of Finland,
University of Jyväskylä, and Okmetic Ltd. After the initial project ended the development has been
continued by CSC in different application fields.

\subsection*{Licensing}
In September 2005 Elmer was released under GPL. This has enabled a significant growth of the 
international user community. However, as the sole owner of the copyright to Elmer source code CSC may distribute Elmer also 
...
\begin{itemize}
  \item Heat transfer: conduction, convection, radiation and phase change
...
of the Elmer package. The list is of course very subjective and not complete either.
\begin{itemize}
\item[+] Elmer is Open Source, therefore it is possible to verify and modify the solution procedures
\item[+] Elmer has an abstract treatment of physical equations making it easy to adapt into new fields
...
\item[+] Elmer is easily compiled for most Unix systems and is also available
  for Windows and Mac machines as precompiled binaries.
...
\item[-] The documentation of Elmer may lag behind the solver development
\end{itemize}
...
and postprocessor but there are also other modules that may called for specific assingments. 
...
\sifitem{ElmerPost}{}
ElmerPost is a versatile postprocessor that is usually quite sufficient for the postprocessing
...
2D Delaunay type of meshes which are obtained by calling the \texttt{Mesh2D} program. 
...
other are used as libraries only. Below is a possible distribution list. The first version number changes in
major new releases while the second number changes about a few times in a year. Basically the user 
does not need knownledge about the different modules unless she wants to setup only a partial system.
\sifbegin
...
the command file interpreter of ElmerSolver and inside the command window of ElmerPost. 
...
Includes the source codes of the 2D Delaunay mesher written in C++. 
\sifitem{umfpack-4.4.tar.gz}{}
Includes the source codes of the Umfpack library from University of Florida.
\sifend
...
Elmer use and create during the solution procedure.
...
\texttt{mesh.elements}, \texttt{mesh.boundary}, and \texttt{mesh.header}
...
This is the format that Mesh2D uses to create Delaunay triangulations. 
...
This file is required by ElmerSolver in case no command line parameter defining the 
name of the command file is given. Then this file simply includes the name of the 
command file.

\sifitem{SOLVER.KEYWORDS}{}
The keywords of ElmerSolver are partially listed in a file located 
...
add some of the missing keywords. Note that it does not really matter if keywords are not defined,
...
Particularly the ElmerSolver Manual is still not ready. The current set of Elmer manuals is as follows
\sifbegin
...
\sifitem{ElmerTutorials.pdf}{}
	...
	The related \texttt{ElmerTutorialFiles.tar.gz}
...
best presented in them. In time we'll try to make these fully obsolete by developing the new set 
of manuals.
\sifbegin
\sifitem{OldElmerUserGuide.pdf}{}
	...
	file formats may still be usefull.
...
Examples on the different strategies may be found in Elmer tutorials. 
Figure~\ref{fig_workflow} shows a schematic picture on how the different components depend
on one-another.
...
The purpose of the preprocessing part is to create the files 
that ElmerSolver needs to run which means at least the command file and the finite element 
mesh. 
...
and the mesh generation tools are limited to 2D Delaunay. 
...
(heat, flow, stress). The good thing is that the user does not need to get acquainted with the 
...
an existing mesh (even 3D). The supported formats include Abaqus, Fidap neutral file, Ideas
universal file and Elmer mesh format. 
After the mesh has been successfully read in the use of ElmerFront proceeds as in the previous case.
...
text editor (e.g. emacs) to account for more exotic details. In adding the keywords the 
Models Manual is of great help. The ElmerSolver and ElmerPost 
...
box like forms and making more complicated shapes may be difficult. The creation of meshes using
...
by ElmerFront (e.g. Ansys, Comsol, Gmsh). This is the line for importing meshes that is currently developed. 
The parsers are often written case-by-case as no documentation of the file formats is often available. 
...
However, complicated geometries are still laborious to setup with a text editor. 
...
ElmerSolver can write the results also in formats understood by third party visualization 
...
\item Malinen Mika, mika.malinen@csc.fi
