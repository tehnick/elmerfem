...
static char RCSid[] =
    "@(#)$Header: /fs/proj1/elmer/elmersrc/sparse/spbuild.c,v 1.2 1999/12/13 16:04:15 jpr Exp $";
#endif
...
register  ElementPtr  pElement;
void Translate();
int IntRow=Row,IntCol=Row;

/* Begin `spClear'. */
    ASSERT( IS_SPARSE( Matrix ) AND Row > 0 );

#if TRANSLATE
    if ((IntRow = Matrix->ExtToIntRowMap[Row]) <= -1)
    {
      fprintf( stderr, "spZeroRow nonexistent row: %d\n ", Row);
      return;
    }
#endif
...
/* Clear matrix. */
    ...
    if (Matrix->PreviousMatrixWasComplex OR Matrix->Complex)
    {
        pElement = Matrix->FirstInRow[IntRow];
        while (pElement != NULL)
        {   pElement->Real = 0.0;
            pElement->Imag = 0.0;
...
ElementPtr spcGetElement(MatrixPtr), spcGetFillin(MatrixPtr);
