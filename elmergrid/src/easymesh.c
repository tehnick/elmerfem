...
static struct ele elem[MAX_NODES*2];
static struct sid side[MAX_NODES*3];
static struct nod node[MAX_NODES],point[MAX_NODES/2];
static struct seg *segment;
static struct chai *chain;

static int Ne,Nn,Ns,Nc;             /* number of: elements, nodes, sides */
static int ugly;                       /* mora li biti globalna ??? */
static char name[80];
static int len;
static double ratio;
...
int insert_node(double x, double y, int spac,
		int prev_n, int prev_s_mark, int mark, 
		int next_s_mark, int next_n)
{
...
+------------*/
 ...
 printf("Easymesh saving %d elements, %d nodes, %d sides.\n",r_Ne,r_Nn,r_Ns);
...
int EasymeshCopy(struct FemType *data,struct BoundaryType *bound)
{
 ...
 if(r_side==NULL) {fprintf(stderr, "Sorry, cannot allocate enough memory !\n");
   return(1);
 }
...
+------------*/
 ...
 data->noknots = r_Nn;
 data->noelements = r_Ne;
 printf("Easymesh copying %d nodes.\n",r_Nn);

 for(n=0; n<r_Nn; n++) {
   data->x[n+1] = r_node[n].x;
   data->y[n+1] = r_node[n].y;
 }
...
+---------------*/
 
 {
   ...
   minmat = maxmat = r_elem[0].material;
  
   printf("Easymesh copying %d elements.\n",r_Ne); 

   for(e=0; e<r_Ne; e++) {
     if(r_elem[e].material < minmat) minmat = r_elem[e].material;
     if(r_elem[e].material > maxmat) maxmat = r_elem[e].material;
   }

   for(e=0; e<r_Ne; e++) {
     if(r_elem[e].material > 0)
       data->material[e+1] = r_elem[e].material;
     else 
       data->material[e+1] = 1;
     
     data->elementtypes[e+1] = 303;
     data->topology[e+1][0] = r_elem[e].i+1;
     data->topology[e+1][1] = r_elem[e].j+1;
     data->topology[e+1][2] = r_elem[e].k+1;
   }
 }
...
+------------*/
 ...
 {
#define BCSIDES MAXBOUNDARIES
   int i,i1,i2,p1,p2,nosides,size,bcsides[BCSIDES],mark;
   nosides = 0;
   ...
   printf("Easymesh copying %d sides, active sides %d.\n",r_Ns,nosides);  
   
   for(i=0;i<BCSIDES;i++) {
     if(bcsides[i]) {
       bound[i].created = TRUE;
       size = bcsides[i];
       bound[i].nosides = size;
       bound[i].types = Ivector(1,size);
       bound[i].areas = Rvector(1,size);
       bound[i].side = Ivector(1,size);
       bound[i].side2 = Ivector(1,size);
   ...
   for(s=0; s<r_Ns; s++) {
     mark = r_side[s].mark;
     if(mark) i = bound[mark].nosides += 1;
     bound[mark].types[i] = mark;
     
     if(r_side[s].ea >= 0) {
       p1 = bound[mark].parent[i]  =  r_side[s].ea+1;
       p2 = bound[mark].parent2[i] =  r_side[s].eb+1;
     }
     else {
       p2 = bound[mark].parent2[i] =  r_side[s].ea+1;
       p1 = bound[mark].parent[i]  =  r_side[s].eb+1;
     }
     i1 = r_side[s].c;
     i2 = r_side[s].d;

     if(p1) {
       if(r_elem[p1-1].i==i1 && r_elem[p1-1].j == i2)
	 bound[mark].side[i] = 0;
       else if(r_elem[p1-1].j == i1 && r_elem[p1-1].k == i2)
	 bound[mark].side[i] = 1;
       else if(r_elem[p1-1].k == i1 && r_elem[p1-1].i == i2)
	 bound[mark].side[i] = 2;
       else if(r_elem[p1-1].i == i2 && r_elem[p1-1].j == i1)
	 bound[mark].side[i] = 0;
       else if(r_elem[p1-1].j == i2 && r_elem[p1-1].k == i1)
	 bound[mark].side[i] = 1;
       else if(r_elem[p1-1].k == i2 && r_elem[p1-1].i == i1)
	 bound[mark].side[i] = 2;

       bound[mark].material[i] = mark;
     }
     if(p2) {
       if(r_elem[p2-1].i == i1 && r_elem[p2-1].j == i2)
	 bound[mark].side2[i] = 0;
       else if(r_elem[p2-1].j == i1 && r_elem[p2-1].k == i2)
	 bound[mark].side2[i] = 1;
       else if(r_elem[p2-1].k == i1 && r_elem[p2-1].i == i2)
	 bound[mark].side2[i] = 2;
       else if(r_elem[p2-1].i == i2 && r_elem[p2-1].j == i1)
	 bound[mark].side2[i] = 0;
       else if(r_elem[p2-1].j == i2 && r_elem[p2-1].k == i1)
	 bound[mark].side2[i] = 1;
       else if(r_elem[p2-1].k == i2 && r_elem[p2-1].i == i1)
	 bound[mark].side2[i] = 2;
     }
   }   
 ...
 printf("Easymesh copying finished\n");
...
void EasymeshInstructions()
{
  ...
  printf("\n   -q ratio  multiply density with ratio"); 
...
int EasymeshExample()
{
  ...
  if( (out=fopen(name, "w"))==NULL ) {
    printf("Can't open file %s for output !",name);
    return(1);
  }
  ...
  fprintf(out, "\n     6:      6     7      3");
  fprintf(out, "\n     7:      7     4      3\n");
  printf("An example file '%s' was created !\n",name);
...
{
 int arg,d=ON,r=ON,s=ON,m=ON,exa=OFF,Nn0;


 if(argc<4) {
   EasymeshInstructions();
   return(0);
 }
 ratio = 1.0;
...
+-----------------------*/
 ...
 for(arg=4; arg<argc; arg++) {
   ...
   if(strcmp(argv[arg],"-q")      ==0) {
     ratio = atof(argv[arg+1]);
     printf("Mapping densities with %.3lg\n",ratio); 
   }
 }
 ...
 if(exa=ON || EasymeshLoad() != 0) {
   printf("The easymesh file %s did not exist therefore one will be automatically created\n",name);
   EasymeshInstructions();
   EasymeshExample();
   return(1);
 }
 ...
 if(m==ON)
   printf("%d Nodes, %d Elements, %d Sides, %d Active sides\n",
	  Nn, Ne, Ns, Nc);
